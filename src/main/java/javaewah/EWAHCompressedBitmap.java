 ...
 * patents.</p>
 */
public final class EWAHCompressedBitmap extends DocIdSet
        implements Bits, Cloneable, Externalizable /* conflict with DocIdSet, Iterable<Integer> */ {
    ...
    public EWAHCompressedBitmap and(final EWAHCompressedBitmap a) {
        final EWAHCompressedBitmap container = new EWAHCompressedBitmap();
        container.reserve(this.actualsizeinwords > a.actualsizeinwords ? this.actualsizeinwords
                : a.actualsizeinwords);
        final EWAHIterator i = a.getEWAHIterator();
        final EWAHIterator j = getEWAHIterator();
        if (!(i.hasNext() && j.hasNext())) {// this never happens...
